...
def get_current_events(feed):
    ...
    try:
        if config['FILES']:
            ...
            cal = events(feed, start=datetime.now() - timedelta(days=config.get('PAST_DAYS_TO_SYNC', 0)),
                         end=events_end)
...
def create_id(uid, begintime, endtime):
    ...
    return re.sub('[^{}]'.format(allowed_chars), '', uid.lower()) + str(arrow.get(begintime).timestamp) + str(
        arrow.get(endtime).timestamp)
...
if __name__ == '__main__':
    ...
    gcal_events = get_gcal_events(service,
                                  from_time=(today - timedelta(days=config.get('PAST_DAYS_TO_SYNC', 0))).isoformat())
    ...
    for ical_id, ical_event in ical_events.items():
        if ical_id not in gcal_event_ids:
            ...
            try:
                ...
                time.sleep(config['API_SLEEP_TIME'])
                try:
                    service.events().update(calendarId=config['CALENDAR_ID'], eventId=gcal_event['id'],
